...
3. **TimeLLM** 🤖 (1-2 секунды без GPU / 30-90 секунд с GPU)
   - 🟢 **По умолчанию: Simple режим** (быстро, без GPU, стабильно)
   - 🟡 **Опционально: NeuralForecast + SLM** (требует GPU, медленнее)
   - **Топовые Small Language Models 2024-2025** (для NeuralForecast):
   ...
   ⚠️ **ВАЖНО**: NeuralForecast режим требует много VRAM и может вызывать OOM ошибки.
   Рекомендуется использовать Simple режим (по умолчанию) для стабильной работы.

4. **Hybrid Model** 🎯 (30-60 секунд без GPU / 1-3 минуты с GPU)
   - **РЕКОМЕНДУЕТСЯ** - лучшее качество!
   - Комбинирует SARIMA-XS + XGBoost + TimeLLM
   - **С GPU**: TimeLLM использует современные SLM (Qwen2-0.5B по умолчанию)
   - **Без GPU**: TimeLLM использует Simple режим (автоматически)
   - Адаптивное взвешивание на основе исторической точности
   - Автоматическая корректировка весов во времени
   - Выбор SLM модели через UI для максимальной гибкости
