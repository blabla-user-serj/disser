...
class HybridModel:
    ...
    T_NORMAL = 30       # Формула 2.9: теоретическая длина нормального ряда (Гл. 2)
    ...
    def _alpha_correction(self, n):
        ...
        α(n) = 1 + ln(T_NORMAL / n)  для n < T_NORMAL (T_NORMAL = 30)
        α(n) = 1.0                   для n >= T_NORMAL

        Численные значения (T_NORMAL = 30):
          При n=5:  α ≈ 2.79 (максимальное усиление штрафа)
          При n=10: α ≈ 2.10
          При n=15: α ≈ 1.69
          При n=20: α ≈ 1.41
          При n=30: α = 1.0  (стандартное взвешивание)

        Нелинейность: при уменьшении n с 30 до 15 коэффициент
        возрастает с 1.0 до ~1.69 (+69%); при уменьшении с 15 до 10
        рост составляет ~24% (от 1.69 до 2.10).
        """
        if n < self.T_NORMAL:
            return 1 + np.log(self.T_NORMAL / n)
        return 1.0
